...
public class KnnPatternClassificator extends CharacterRecognizer {
    Map<String,Vector<Double>> learnVectors;

    // Altered by Kyle Atkinson 2011.11.15
    // Allow multiple reference images for each letter
    public KnnPatternClassificator() throws IOException {
        ...
        this.learnVectors = new HashMap<String,Vector<Double>>();
        ...
        for (String fileName : folder.list()) {
            ...
            testString += alphaString.substring(alphaPosition, alphaPosition+1);
            ...
            this.learnVectors.put(fileName, imgChar.extractFeatures());
        }

        // kontrola poloziek vektora
        //for (int i=0; i<alphaString.length(); i++)
        for (char c : alphaString.toCharArray())
            if (testString.indexOf(c)==-1) throw new IOException("Warning : alphabet in "+path+" is not complete");
    ...
    public RecognizedChar recognize(Char chr) throws Exception {
        ...
        for (String key : this.learnVectors.keySet()) {
        //for (int x = 0; x < this.learnVectors.size(); x++) {
            ...
            float fx = this.simplifiedEuclideanDistance(tested, this.learnVectors.get(key));
            ...
            recognized.addPattern(recognized.new RecognizedPattern(key.toUpperCase().charAt(0), fx));
